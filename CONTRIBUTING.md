...
| [`sync-formulas.yml`](.github/workflows/sync-formulas.yml) | Weekly schedule (Sun 23:00 UTC) · manual dispatch | Detect new upstream versions · bump version + regenerate gem resources with `gen-formula` · open one PR |
...
- **Bottle freshness:** each formula carries a `# bottle-source-digest:` line
  above its bottle block — a hash of the formula's install-relevant content
  (everything except the bottle block). `bottle.yml` rebuilds the bottle
  whenever that digest changes, so a same-version gem-resource update (from
  `gen-formula`) re-bottles correctly rather than serving a stale bottle.
...
Gem `resource` blocks are generated by `script/gen-formula` (see above) and
regenerated automatically by `sync-formulas.yml`, which now bumps the source
version AND regenerates the gem closure in one weekly PR. There is no separate
gem-sync workflow.
...
- `script/formula-digest` — prints (or `--stamp` writes) a formula's bottle-source-digest; used by `bottle.yml` and the `merge-bottle` action
